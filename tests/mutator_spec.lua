...
a.describe("mutator", function()
  ...
  describe("build actions", function()
    ...
    it("constructs MOVE actions to non-existing directories", function()
      local file = test_adapter.test_set("/foo/a.txt", "file")
      vim.cmd.edit({ args = { "oil-test:///foo/" } })
      local bufnr = vim.api.nvim_get_current_buf()
      local diffs = {
        { type = "delete", name = "a.txt", id = file[FIELD_ID] },
        { type = "new", name = "bar/baz/a.txt", entry_type = "file", id = file[FIELD_ID] },
      }
      local actions = mutator.create_actions_from_diffs({
        [bufnr] = diffs,
      })
      assert.are.same({
        ...
        },
        {
          ...
          entry_type = "file",
          src_url = "oil-test:///foo/a.txt",
          dest_url = "oil-test:///foo/bar/baz/a.txt",
        },
      }, actions)
    end)
